...
usage() {
    NAME=`basename $0`
    cat <<EOF
Usage: ${NAME} [-r|-c|-s|-m|-h] [-d|-t] [-g] DATE [COMMENT]
...
The "-t" option runs for retry clients only.
...
GENERATE=0
DAILY=
while getopts "crsmndtgh" OPT; do
    case "$OPT" in
        r)
            TYPE=Site
            ;;
...
run-main com.krux.marketer.reachoverlap.pipeline.ReachOverlap${DAILY}${TYPE}Pipeline ${COMMAND}
exit
EOF
